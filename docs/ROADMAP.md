...
- Structured template field types (unified text, number, date, time, checkbox, yes/no, dropdown) with stable UUID field/option ids, per-type note controls, id-keyed typed answers, dropdown-label resolution (no id leaks into fields), and read-time backward compatibility (legacy/`multiline` rows render as the unified Text field) — see `docs/ARCHITECTURE.md` → Template System
...
- Continue phased implementation of the approved Template Architecture (see `docs/PROJECT_DECISIONS.md`). Landed so far: the seed migration (Sprint 1), the Template Library cutover (Sprint 2 — library CRUD, default template, immutable versions, per-note pinned instances with a template selector; this closed the former data-orphaning risk), and the structured field-type system (Sprint 3 — seven field types incl. a unified Text field, stable UUID field/option ids, id-keyed typed answers, dropdown-label resolution). Next increments: **template-instance export** (see high-priority item below), per-row photo/file/signature controls, template versioning UI, IndexedDB-backed assets, and export version-provenance.
- **Template-instance export (high priority).** Template field values are currently **not included in any export** — exports render only the rich-text editor HTML (`src/lib/exportUtils.js` via `getNoteContent`). Rendering a note's pinned template instance (labels + typed answers) into PDF/DOCX/HTML/MD is a dedicated export feature: it needs its own export-content pipeline (checkbox → Yes/No, yes/no → Yes/No/blank, dropdown → resolved option label, date/time → stable readable, blank → blank) and should carry version provenance. Deliberately deferred out of the field-type phase to avoid bundling an export-architecture change into it.
