...
| Template library | Named template records and their immutable version snapshots (layout, labels, widths, logo, and per-row **field type + dropdown options**), plus a default-template pointer | Editing a template publishes a new version; existing versions are never rewritten |
| Note template instances | Per-note template answers and attachments, pinned to the specific template version the note was created against | Notes render from their pinned version, not the live template; answers are keyed by stable field id and typed per the field's type (see Template Field Types) |
...
The template system implements the approved Template Library model (multiple named templates, immutable versions, per-note pinned instances). Its data model, storage keys, and immutability rules are in `src/lib/templateModel.js` and the Storage table above.

**Template Field Types.** Each template row carries a structured field `type`, defined and normalized in one place (`src/lib/templateFields.js`). Seven types are supported: **text**, **number**, **date**, **time**, **checkbox**, **yes/no**, and **dropdown** (select). The row shape is `{ id, label, px, minPx, type, options }`; `options` (`[{ id, value }]` with stable ids) is only meaningful for dropdowns.

- **The Text field is unified** — there is no separate single-line/multiline distinction (NoteWise is document-first). Text renders as a full-cell textarea: it fills the right-hand cell, grows as the row is dragged taller, has no inner box or border, preserves line breaks, supports BottomBar insertion, and is the default type for new rows. This keeps the product simple and leaves the field suitable as a future voice-to-field capture destination.
- **Field ids** are stable UUID-style ids (`crypto.randomUUID()` with the safe fallback in `src/lib/id.js`) for newly created rows and dropdown options; existing ids — including older non-UUID ids — are preserved unchanged. Answers are keyed by field id, never by label or row position, so renaming a field or publishing a new version never orphans an answer.
- **Backward compatibility is read-time, not a stored rewrite.** `normalizeRow`/`normalizeRows` supply rendering defaults when a stored version predates the current field-type system, **without** mutating the stored immutable version. Two deliberate compatibility choices: (1) a row with no `type`, **and any older `type: "multiline"`, normalizes to the unified `text`** — because both were rendered as textareas, this keeps existing multi-line answers (including line breaks) visible and unchanged; (2) an id-less legacy row falls back to a **deterministic** positional id (`row-<index>`), never a freshly generated id on read, so answers keyed by it stay stable across reloads. Because these are read-time defaults, **no new storage migration was required**; the existing v2 template migration is unchanged.
- **Answer value types** preserve the distinctions that survive `localStorage` serialization: number is stored as a string so empty (`""`) and zero (`"0"`) stay distinct and invalid text is never coerced to 0; checkbox is a real boolean (`false` is retained, not dropped); yes/no is `"yes" | "no" | ""` (unanswered is never silently "no"); dropdown stores the **stable option id** (the label is resolved from the pinned version, so renaming an option in a newer version never corrupts a note pinned to an older version).
- **No internal id is ever shown as visible field content.** A dropdown answer stores an option id; that id is resolved to its label for display, and an unresolved option id renders blank. A value that is provably an internal id — the field's own id, or a **known dropdown option id** (`collectKnownOptionIds()` scans every version) — is displayed as blank by `displayTextValue`, so an option id can never leak into a text field (e.g. when a field that used to be a dropdown is now Text). This resolution is **display-only and non-destructive**: the stored answer is preserved, so it resolves to a label again if the note returns to a dropdown version. The check is by exact id identity, never by "looks like a UUID", so legitimate user text — even a UUID a user actually typed — is never stripped. No persisted-data repair was needed.
- **Builder vs. note rendering** reuse one component (`ResizableTwoColTable`): the builder shows a per-row field-type selector plus a dropdown-options editor (add/rename/delete, empty values dropped on publish, order preserved, stable ids); the note view renders the type-appropriate control. Publishing writes a new normalized immutable version; existing notes stay pinned to their old version and answers.
- **BottomBar insertion** targets only the free-text field; structured fields reject inserted text rather than being corrupted.

**Template-instance export is currently unsupported.** Exports render only the rich-text editor HTML (`src/lib/exportUtils.js` via `getNoteContent`); template field values are not included in any export today. Wiring template instances into PDF/DOCX/HTML/MD exports is a dedicated export feature (see `docs/ROADMAP.md`), not part of the field-type phase.
...
- The template system implements the approved Template Library model (multiple named templates, immutable versions, per-note pinned instances with a per-note template selector), so editing a master template no longer affects existing notes. The structured **field-type system** (seven types, with a single unified Text field) and **stable UUID ids for builder-added rows/options** are now implemented (see Template System above). Not yet implemented from the approved decision: version history UI, per-row photo/file/signature controls, template-instance export, and IndexedDB-backed template assets — template logos and note attachments are still stored as base64 in localStorage, and superseded versions accumulate without pruning, so storage-quota pressure grows with template count and edit frequency.
