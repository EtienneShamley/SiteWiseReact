...
Since 2026-08-29 NoteWise requires a Firebase project for sign-in: the backend needs `FIREBASE_PROJECT_ID` (nothing else — no service-account key), and the frontend build needs `REACT_APP_FIREBASE_API_KEY`, `REACT_APP_FIREBASE_AUTH_DOMAIN`, `REACT_APP_FIREBASE_PROJECT_ID` and `REACT_APP_FIREBASE_APP_ID` from the project's web-app settings. Without them the app shows a "Sign-in is not configured" notice. Since 2026-09-01 the project also needs **Firestore (Native mode)** with the repository's `firestore.rules` deployed (`npx firebase deploy --only firestore:rules`): a signed-in user's notes, templates and PDF entries are saved to their account's workspace; images, PDF files and attachments stay on the device until a later phase. Optional for local work: `REACT_APP_FIREBASE_FIRESTORE_EMULATOR_HOST` (e.g. `127.0.0.1:8080`) with `npx firebase emulators:start --only firestore,auth`. `npm run test:rules` runs the Security Rules suite in the emulator (needs a JDK ≥ 21). See [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md#environment-variables) for the per-environment table and the one-time Firebase Console steps.
